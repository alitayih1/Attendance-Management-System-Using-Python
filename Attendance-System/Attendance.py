...
#MyEngine = sqlalchemy.create_engine('Attendance://root:root@localhost/Attendance', echo = True)
#engine.execute("CREATE DATABASE Attendance") #create db
...
message = tk.Label(window, text="Face Recognition Attendance Management System" ,bg="Blue"  ,fg="white"  ,width=50  ,height=3,font=('times', 30, 'italic bold'))
...
def TakeImages():
    ...
    if(is_number(Id) and name.isalpha()):
        ...
        #to capture images using the webcam and process them and detect faces.
        ...
        while(True):
            ...
            rgb  = img
            #detect all faces on the current frame
            faces = detector.detectMultiScale(gray, 1.3, 5)
            #on faces we will detect many faces and put int into rectangle
            for (x,y,w,h) in faces:
                cv2.rectangle(img,(x,y),(x+w,y+h),(255,0,0),2)
                #incrementing sample number
                sampleNum=sampleNum+1
                #saving the captuwhite face in the dataset folder TrainingImage
                cv2.imwrite("originalImages\ "+name +"."+Id +'.'+ str(sampleNum) + ".jpg",img[y:y+h,x:x+w])
...
takeImg = tk.Button(window, text="Take Images", command=TakeImages  ,fg="white"  ,bg="blue"  ,width=20  ,height=3, activebackground = "white" ,font=('times', 15, ' bold '))
takeImg.place(x=100, y=500)
trainImg = tk.Button(window, text="Train Images", command=TrainImages  ,fg="white"  ,bg="blue"  ,width=20  ,height=3, activebackground = "white" ,font=('times', 15, ' bold '))
trainImg.place(x=400, y=500)
trackImg = tk.Button(window, text="Track Images", command=TrackImages  ,fg="white"  ,bg="blue"  ,width=20  ,height=3, activebackground = "white" ,font=('times', 15, ' bold '))
...
copyWrite.insert("insert", "Developed by Ali","", "TEAM", "superscript")
